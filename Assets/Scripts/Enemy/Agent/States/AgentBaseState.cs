...
public abstract class AgentBaseState
{
    ...
    protected void LogAgentStatus(AgentController agent, string action)
    {
        ...
        // 追加：AITypeを取得
        var aiType = agent.GetData().aiType;

        // Console 出力
        Debug.Log(
            $"[AgentLog] {agent.name} | State={stateName} | Action={action} | Type={aiType} | Target={targetName}"
        );
...
//    //    // Console 出力
//    //    Debug.Log($"[AgentLog] {agent.name} | State={stateName} | Action={action} | Type={aiType} | Target={targetName}");
...
//    //        targetPos,
//    //        self,
//    //        aiType.ToString() // ← 追加（AITypeを文字列で記録）
//    //    );
//    //}
